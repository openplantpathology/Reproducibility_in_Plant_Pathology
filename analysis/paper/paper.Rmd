...
These classifications were based on the individual evaluator's judgement of the article.

Each article was rated on a 0 -- 3 scale for its software, data and code availability.
'Software availability' was defined as follows, a score of '0' was 'Not available or not mentioned in the publication'; '1' was 'Expensive proprietary software that only institutions would typically purchase'; '2' was 'Used proprietary software that most individuals can afford (e.g., Excel) or SAS that is now available free for use with a university login'; and '3' was 'Used entirely open source and free software (e.g., R, Julia, Python)'; 'NA' indicates that no specialised software was necessary or used.
...
Efforts were made to standardise the scoring as much as possible.
...
The articles' computational reproducibility were analysed using Bayesian mixed-effect models fit using the contributed package 'brms' (version `r packageVersion("brms")`) [@Burkner2017; @Burkner2018; @Burkner2021] to evaluate the fixed effects of the publication (journal title), five-year journal impact factor, journal open access policy, journal data and code deposition instructions on three factors, data and code availability how open and available the software used were.
The models included 'year', 'journal' and 'assignee' as random effects except in the year of publication analysis where 'year' was a fixed effect, publication analysis where the 'journal' was a fixed effect and five-year impact factor where correlations between the journal title and impact factor affected the software availability model.
...
Each model was run for 4000 iterations in four chains.
...
The differences between the co-authors' classifications of the article class as `applied` or `fundamental` had no differences in the five articles that were evaluated by all of the co-authors (Figure \@ref(fig:irArtClass)A).
However, there were several differences in evaluators' interpretations of whether or not an article had a molecular focus (Figure \@ref(fig:irArtClass)B).
All authors agreed on the evaluation of the computational methods and data availability of all five articles that were scored by each author (Figure \@ref(fig:irCriteria)A, \@ref(fig:irCriteria)B).

The analysis detected that the journal title did have an effect on the reproducibility score but there was no effect of article class or impact factor.
...
While the analysis did not detect an effect of the journals' instructions for sharing or depositing data and or code, the more detailed the instructions, the higher the article scored.
...
```{r irArtClass, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Evaluator classifications of (A) an articles focus being molecular techniques (TRUE/FALSE) and (B) an article's class (Fundamental/Applied). Where all five articles were scored by all five authors to check for agreement between the evaluations.", cache=FALSE, fig.env='figure'}

theme_set(theme_classic())
a <- ggplot(ir, aes(y = doi,
                      fill = molecular)) +
  geom_bar() +
...
p <- p + plot_annotation(tag_levels = "A",
                         title = "Evaluator agreement in article classifications")
