...
institute:
  ...
  - ARS: Horticulural Crops Research Unit, USDA Agriocultural Research Service, Corvallis, OR, USA
...
The highest level includes efforts to annotate structured raw data (FAIR -- Findable, Accessible, Interoperable and Re-usable) and fully document the analysis using open source code which are deposited in public repositories and can be run by anyone following download of data and code.
...
In many cases, data sets are small enough and curated in spreadsheets, which if saved as a plain text file, _e.g._ comma separated (CSV) files, allows for additions or updates to the data can be tracked through a version control system such as Git, Mercurial among others.
Ideally, once the data are complete, best practices for keeping your data as you perform your work include treating the raw data as read-only, using file permissions to prevent changes to the raw data files or possibly where appropriate using a data base to store the data that have been collected.
...
If steps are followed to make the data FAIR (Findable, Accessible, Interoptable and Reusable) [@Wilkinson2016] then it will be readable by humans and machines alike this will help support new discoveries and support further research.
In turn sharing data will lead to new citations for your work as other discover and use it.

To make your data the most widely useable, ensure that it has a persistent identifier.
...
Twenty-one plant pathology discipline journals were selected by the authors as representations of discipline-based journals target by the plant pathology research community.
Among them, both fundamental and/or applied as well as journals covering specific group of pathogens/plants or broad areas were included. Two hundred articles were randomly selected from issues published from 2012 to 2018.
...
In cases where an article was not suitable, _e.g._, a review or otherwise not related to plant pathology, the next article was selected until a suitable article was found. Notes regarding the selection of articles can be found in the file, XXXX, available in this paper's repository.
...
The list of journals was saved as a comma separated value (CSV) file and imported into R [@RCT2020]. 
