...
The analysis found that publications in Phytopathology were more likely to share their code but undecided on any effects of any other journal titles (Median = `r round(phytopath_draws_f1$median, digits = 2)`, 95% CI [`r round(phytopath_equiv_f1$HDI_low, digits = 2)`, `r round(phytopath_equiv_f1$HDI_high, digits = 2)`] and can be considered as significant (`r round(phytopath_equiv_f1$ROPE_Percentage, digits = 2)`% in ROPE)).
Publications in three journals, Crop Protection, Plant Disease and Revista Mexicana de Fitopatología were not likely to share their data (Median = `r round(cropProt_draws_f2$median, digits = 2)`, 95% CI [`r round(cropProt_equiv_f2$HDI_low, digits = 2)`, `r round(cropProt_equiv_f2$HDI_high, digits = 2)`] with (`r round(cropProt_equiv_f2$ROPE_Percentage, digits = 2)`% in ROPE), Median = `r round(plantDis_draws_f2$median, digits = 2)`, 95% CI [`r round(plantDis_equiv_f2$HDI_low, digits = 2)`, `r round(plantDis_equiv_f2$HDI_high, digits = 2)`] with (`r round(plantDis_equiv_f2$ROPE_Percentage, digits = 2)`% in ROPE) and Median = `r round(revMexFitopatol_draws_f2$median, digits = 2)`, 95% CI [`r round(revMexFitopatol_equiv_f2$HDI_low, digits = 2)`, `r round(revMexFitopatol_equiv_f2$HDI_high, digits = 2)`] (`r round(revMexFitopatol_equiv_f2$ROPE_Percentage, digits = 2)`% in ROPE), respectively, and so can be considered significant but no effects were detectable in any other journal titles.
...
However, of the articles surveyed those published in 2016 did share more code than in other years (Median = `r round(year2016_draws_g2$median, digits = 2)`, 95% CI [`r round(year2016_equiv_g2$HDI_low, digits = 2)`, `r round(year2016_equiv_g2$HDI_high, digits = 2)`] and can be considered as significant (`r round(year2016_equiv_g2$ROPE_Percentage, digits = 2) * 100 `% in ROPE)), but no discernible pattern was found across years.
...
But in those journals where the policy was fully open access, the articles tended to be more likely to share code (Median = `r round(access_draws_i2$median, digits = 2)`, 95% CI [`r round(access_equiv_i2$HDI_low, digits = 2)`, `r round(access_equiv_i2$HDI_high, digits = 2)`]).
...
equivalence_test(m_f1) %>%
  ...
  set_formatter(`ROPE %` = function(x) sprintf("%1.f%%", x * 100)) %>%
  autofit() %>%
  set_caption(
