...
We randomly selected 450 articles published from 2012 to 2021 across 21 journals representative of the plant pathology discipline and assigned them scores reflecting their openness and reproducibility.
...
The articles' scores for data, code and software availability were analysed using Bayesian mixed-effect models fit using the contributed package 'brms' (version `r packageVersion("brms")`) [@Burkner2017; @Burkner2018; @Burkner2021] to evaluate the fixed effects of the publication (journal title), five-year journal impact factor, year of publication, journal open access policy, and the effect of the availability of the software used, `software_avail` on the code and data availability.
The models included 'year', 'journal' and 'assignee' as random effects except in the year of publication analysis where 'year' was a fixed effect, journal title analysis where the 'journal' was a fixed effect and the five-year impact factor model where correlations between the journal title and impact factor affected the software availability model.

Non-informative priors were selected from a normal distribution with a mean of 0 and a standard deviation of 1.5 were selected for all models.

Models were fit using the `cumulative(link = "logit")` family function for ordinal scale data.
Each model was run for 10,000 iterations in four chains with a warmup of 3000 iterations and thinning set to 1.
...
The remainder of the top ten programs we found were related to sequence analysis, *i.e.*, Clustal (`r scales::ordinal(which(tab_s == "CLUSTAL"))`), BLAST (`r scales::ordinal(which(tab_s == "BLAST"))`), and BioEdit (`r scales::ordinal(which(tab_s == "BIOEDIT"))`) or phylogenetic analysis MrBayes (`r scales::ordinal(which(tab_s == "MRBAYES"))`).
The most general-purpose software cited in the top 10 was Excel® (`r scales::ordinal(which(tab_s == "EXCEL"))`).
...
The remainder of the top ten software packages we found were related to sequence analysis, *i.e.*, BLAST, Clustal and BioEdit, phylogenetic analysis MrBayes or the general-purpose spreadsheet program Excel®.
