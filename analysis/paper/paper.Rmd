...
A list of randomly generated page numbers from one to 150, sampled with replacement, was assigned to a randomized list of the 21 journals.
...
A custom R function, `import_notes()`, was written to import the data, format the columns properly and calculate the overall reproducibility score in R [@RCT2022].
...
The statistical analysis was performed using R version 4.1.0 [@RCT2022] on an Apple MacBook Air (M1, 2020).
Bayesian mixed-effect models were fit using the contributed package 'brms' (version 2.16.3) [@Burkner2017; @Burkner2018; @Burkner2021] to evaluate the fixed effects of the publication (journal title), article class, molecular focus, five-year journal impact factor, journal open access policy on the reproducibility index and year to test for changes over time in the reproducibility index.
...
Model finesses were evaluated using model summaries and diagnostic plots from 'brms' (version 2.15.0) [@Burkner2018] and posterior fits using `pp_check()`  (Figure \@ref(fig:posteriors)) from the contributed R package 'bayesplot' (version 1.8.1) [@Gabry2019].
...
Data were visualised using 'ggplot2' (version 3.3.5) [@Wickham2016], 'ggpubr' (version 0.4.0) [@Kassambara2020] and patchwork (version 1.1.1) [@Pedersen2020].
...
The fourth level includes packaging the analysis as a software package, _e.g._, R, Julia or Python package at a minimum, with the use of continuous integration and possibly a software container like Docker as Khaliq et al. [@Khaliq2020] have done.
