...
Statistical analysis were carried out using R 4.1.0 (R Core Team, 2021), the rstanarm (v2.13.1; Gabry & Goodrich, 2016) and the report (v0.2.0; Makowski, Patil, & Lüdecke, 2019) packages. The full reproducible code is available in @Sparks2022.

Inter-rater differences were evaluated using percent agreement and Fliess' Kappa [@DelPonte2019].

We fitted Bayesian logistic mixed models (estimated using MCMC sampling with 4 chains of 10000 iterations and a warmup of 5000 and thinning of 1) using cumulative family function for ordinal data using the contributed package 'brms' (version `r packageVersion("brms")`) [@Burkner2017; @Burkner2018; @Burkner2021].
...
The best fitting models were selected by using Widely Applicable Information Criterion (WAIC) and the expected log pointwise predictive density via `loo_compare()`.

The models' were structured as follows to predict two responses, the availability of computational methods and data that were created or collected for the manuscript.
...
Model fitnesses were evaluated using model summaries and diagnostic plots from 'brms' (version `r packageVersion("brms")`) [@Burkner2017; @Burkner2018; @Burkner2021] and posterior fits using `pp_check()` (Figure \@ref(fig:posteriors)) from the contributed R package 'bayesplot' (version `r packageVersion("bayesplot")`) [@Gabry2019].
...
Convergence and stability of the Bayesian sampling had been assessed using R-hat, which should be below 1.01 [@VVehtari2021] and Effective Sample Size (ESS), which should be greater than 1000 [@Burkner2017].

A full report of all parameters and model details are available in the Supplementary Materials.
...
All authors agreed on the five inter-rater article evaluations for the data availability, '0', not available for a inter-rater score of 100%.
...
When predicting the availability of data availability as compared with *Phytopathology*, publications in three journals, *Crop Protection*, *Plant Disease* and *Revista Mexicana de Fitopatología*, can be considered clearly less likely to share data than publications in *Phytopathology*, but no effects were detectable in any other journal title.
...
```{r year-parameters, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='95% credible interval for the probability associated with the year of publication for (A) computational materials (code) and (B) data being made readily available to the public.', cache=FALSE, fig.env='figure'}
...
```{r IF-parameters, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='95% credible interval for the probability associated with the publishing journal\'s five-year impact factor for (A) computational materials (code) and (B) data being made readily available to the public.', cache=FALSE, fig.env='figure'}
