...
```{r equivalence-tests, echo=FALSE, warning=FALSE, message=FALSE}
a <- plot(equivalence_test(m_f1), rope_color = "grey70") +
  scale_y_discrete(
    label = function(x) {
      x %>% gsub("b_abbreviation", "", .)
    }
  ) +
  scale_fill_grey()
b <-
  plot(equivalence_test(m_f2), rope_color = "grey70") +
  theme(
  ...
  scale_fill_grey()
c <-
  plot(equivalence_test(m_f3), rope_color = "grey70") +
  theme(
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.title.y = element_blank()
