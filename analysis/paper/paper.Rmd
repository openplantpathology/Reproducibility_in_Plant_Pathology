...
To predict the responses with publishing journal, as the journals' abbreviation, the formula took the form: response ~ abbreviation and the models included assignee as a random effect (formula: ~1 | assignee).
...
Model fitness's were evaluated using model summaries and diagnostic plots from 'brms' (version `r packageVersion("brms")`) and posterior fits using `pp_check()` (Figure \@ref(fig:posteriors)) from the contributed R package 'bayesplot' (version `r packageVersion("bayesplot")`) [@Gabry2019].
