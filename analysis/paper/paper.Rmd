...
In some cases the `software_avail`, or the openness of the software used, score did not agree with the software that was recorded and these scores were modified after discussion to match the agreed on ratings.
For example, a paper was rated as having a `software_avail` rating of `3`, but the evaulator noted that Excel had been used.
Here, the `software_avail` rating was changed to `2` to bring it into line with the agreed upon guidelines.
...
In these cases, Excel was added to the `software_used` column.
...
However, differences in evaluators' interpretations of whether or not an article had a molecular focus or not led to this scoring criteria being dropped from the analysis and is not included in this analysis (Figure \@ref(fig:irArtClass)B).

There was reasonable agreement among the authors when evaluating whether software was cited properly or not and freely and openly available (Figure \@ref(fig:irCriteria)A, \@ref(fig:irCriteria)B).
All authors agreed on the evaluation of the computational methods and data availability of all five articles that were scored by each author (Figure \@ref(fig:irCriteria)C, \@ref(fig:irCriteria)C).

The overall reproducibility scores assigned by authors to the five articles evaluated to check for inter-rater differences showed some differences (Figure \@ref(fig:irReproScore)).
The article that had the greatest difference, 13 to 33%, was DOI "10.1007/s12600-021-00902-2" with a standard deviation of `r ir %>% filter(doi == "10.1007/s12600-021-00902-2") %>% summarise(x = round(sd(reproducibility_score), 1))`.
All other articles had standard deviation scores of less than 4 (Table \@ref(tab:interRaterSD)).
However, all article scores largely agreed and fell below 50% (Figure \@ref(fig:irReproScore)).

There were `r length(which(rrpp$reproducibility_score == 100))` (`r round(length(which(rrpp$reproducibility_score == 100))/nrow(rrpp), 2) * 100`%) articles that scored 100% or were classed as fully reproducible (Figure \@ref(fig:reproScore)).
`r toTitleCase(as.character(english::english(length(which(rrpp$reproducibility_score == 0)))))` (`r round(length(which(rrpp$reproducibility_score == 0))/nrow(rrpp), 2) * 100`%) of articles were scored at 0% out of a possible 100% for reproducibility (Figure \@ref(fig:reproScore)).
...
The scripts provided used an R package, _checkpoint_ [@Ooi2022], to provide a mechanism to help ensure reproducibility by installing the package versions used by the authors that are necessary to reproduce the work rather than defaulting to the latest versions.
...
```{r interRaterSD, echo=FALSE, message=FALSE, warning=FALSE}
ir %>%
  group_by(doi) %>%
  summarise(x = round(sd(reproducibility_score), 1)) %>%
  rename(`Standard Deviation` = x) %>%
  rename(DOI = doi) %>%
  flextable() %>%
  set_table_properties(layout = "autofit") %>%
  set_caption(
    "Standard deviation of reproducibility scores for five articles evaluated by all five co-authors to check for differences in article ratings.")
```
...
load(file = here("inst/extdata/m_f.Rda"))
load(file = here("inst/extdata/m_g.Rda"))
load(file = here("inst/extdata/m_h.Rda"))
load(file = here("inst/extdata/m_i.Rda"))
load(file = here("inst/extdata/m_j.Rda"))
load(file = here("inst/extdata/m_k.Rda"))
...
p <- (pp_m_f | pp_m_g) / (pp_m_h | pp_m_i) / (pp_m_j | pp_m_k)
...
```{r irReproScore, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Overall reproducibility scores of five articles scored for evaluator agreement. Where 0 was not reproducible in any fashion and a perfect score would be 100% as evaluated for four areas software citations, software availability, computational methods availability and data availability on a 0 to 3 scale and converted to a percent of total possible score value.", cache=FALSE, fig.env='figure'}
p <- ggplot(ir,
            aes(y = doi,
                x = reproducibility_score)) +
  geom_density_ridges() +
  ggtitle("Inter-rater reproducibility score for five articles") +
  ylab("DOI") +
...
```{r reproScore, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Reproducibility scores of 450 papers published in 21 plant pathology journals or plant pathology focused articles from other specialised journals. Where 0 was not reproducible in any fashion and a perfect score would be 100% as evaluated for four areas software citations, software availability, computational methods availability and data availability on a 0 to 3 scale and converted to a percent of total possible score value.", cache=FALSE, fig.env='figure'}
p <-
...
p <- ggplot(rrpp_long, aes(x = as.factor(value))) +
  ...
  ylab("Count") +
  xlab("Article Score") +
  ggtitle("Reproducibility criteria index values") +
  facet_wrap(name ~ .)
p <- tag_facet(
  p = p,
  tag_pool = LETTERS,
...
ggsave(
  filename = here("analysis/figures/Sparks_et_al_Figure_6.eps"),
  plot = p,
  width = 6,
  height = 6,
  units = "in",
...
rsvg_ps(svg = charToRaw(p),
        file = here("analysis/figures/Sparks_et_al_Figure_7.eps"))
embed_fonts(
  file = here("analysis/figures/Sparks_et_al_Figure_7.eps"),
  outfile = here("analysis/figures/Sparks_et_al_Figure_7.eps"),
  options = "-dEPSCrop"
)

include_graphics(path = here("analysis/figures/Sparks_et_al_Figure_7.eps"))
```
