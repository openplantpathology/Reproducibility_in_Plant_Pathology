...
Models 1 and 2 were used to predict responses with publishing journal, using the journals' abbreviation as the predictor the formula took the form: response \~ abbreviation (model 1: code_availability \~ abbreviation and model 2: data_availability \~ abbreviation) and the models included assignee as a random effect (formula: \~1 \| assignee).
...
Models 5 and 6 were used to predict the responses using IF_5year as the predictor the formula took the form: response \~ IF_5year and included assignee as random effect (formula: \~1 \| assignee) without journal as a random effect to mitigate correlation between impact factor and the journal of publication.
...
When predicting the availability of computational methods (code) and availability, all journals were compared with *Phytopathology*&reg; as the reference with latent scores having a mean of zero.

There were no clear differences that the analysis could detect between any of the journals as compared with *Phytopathology*&reg; for the sharing of computational methods (code); the effects of all parameters were undecided (Figure \@ref(fig:journal-parameters)A, Table S1, Figure S2).

When predicting the availability of data availability as compared with *Phytopathology*&reg;, publications in one journal, *Phytopathologia Mediterranea*, can be considered clearly more likely to share data than publications in *Phytopathology*&reg;, but no effects were detectable in any other journal title (\@ref(fig:journal-parameters)B, Table S2, Figure S3).

The effect of *Phytopathologia Mediterranea* (Median = 1.68, 95% CI [0.78, 2.57]) has a 99.99% probability of being positive (> 0), 99.98% of being significant (> 0.05), and 99.83% of being large (> 0.30). The estimation successfully converged (Rhat = 1.000) and the indices are reliable (ESS = 19017).
...
This approach assumed that there was no effect or bias on reproducibility based on the time of year that an article was published since most journals start with page number one at the beginning of the year and some journals, _e.g._, *Phytopathology*&reg;&reg; have focus issues with invited authors.
...
Authors publishing in *Phytopathologia Mediterranea* were more likely than articles published in *Phytopathology*&reg; to share data.
