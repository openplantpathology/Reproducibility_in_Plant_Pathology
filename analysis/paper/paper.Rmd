...
Additionally, data that supported the articles were mostly unavailable with `r length(which(rrpp$data_avail == 0))` (`r round(length(which(rrpp$data_avail == 0))/nrow(rrpp), 2)`%) scoring '0' where the data was not available or mentioned in the article.
...
When predicting the availability of computational methods (code) and availability, all journals were compared with *Phytopathology* as the reference with latent scores with a mean of zero.

There were no clear differences that the analysis could detect between any of the journals as compared with *Phytopathology* for the sharing of computational methods (code); the effects of all parameters were undecided (Figure \@ref(fig:journal-parameters)A).
...
Publications in three journals, *Crop Protection*, *Plant Disease* and *Revista Mexicana de Fitopatología*, can be considered clearly less likely to share data than publications in *Phytopathology*, but no effects were detectable in any other journal title.
