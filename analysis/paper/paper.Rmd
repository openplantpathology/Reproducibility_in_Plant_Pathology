...
We fitted Bayesian logistic mixed models (estimated using MCMC sampling with 4 chains of 10000 iterations and a warmup of 5000 and thinning of 1) using the `cumulative(link = "probit")` family function for ordinal scale data using using the contributed package 'brms' (version `r packageVersion("brms")`) [@Burkner2017; @Burkner2018; @Burkner2021].
Priors were selected to be weakly informative and deemed suitable through using `pp_check()` to examine the predictions based on priors only (using the parameter `sample_prior = only` in `brm()`).
Priors over all parameters were set as: normal (mean = 0.00, SD = 1.0) distributions for both fixed parameters and random intercepts.
...
For brevity, only details of parameter where the 95% CI fell outside the Region of Practical Equivalence (ROPE) are reported here.
For a full description of all models' parameter details see (XXXXXX).

When predicting the availability of computational methods (code) availability, all journals were compared with *Phytopathology*
There were no clear differences found between any of the journals as compared with *Phytopathology* for the sharing of computational materials, the effects of all parameters were undecided.

When predicting the availability of data availability as compared with *Phytopathology* two journals were not likely to share data, with the others being unable to detect any effect.
...
No journals surveyed were completely closed, all offered at least an option for open access articles but `r rrpp %>% distinct(abbreviation, open) %>% filter(open == TRUE) %>% nrow()` (`r round(rrpp %>% distinct(abbreviation, open) %>% filter(open == TRUE) %>% nrow()/nrow(rrpp), 2)`%) were completely open.
