...
```{r new_assignees}
org_new_assigneess <- rep(c("Emerson", "Kaique"), 50)
```
...
journals <- cbind(journals[, -1], year, contains_page, new_assignees)

journals <- arrange(.data = journals, new_assignees, journal, year, contains_page)
...
article_notes <- gs_title("new_article_notes")

notes <- 
  article_notes %>% 
    gs_read(ws = "new_article_notes")
