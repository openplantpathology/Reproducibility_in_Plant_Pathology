...
vignette: >
  %\VignetteIndexEntry{Assigning Articles to Evaluators}
...
Twenty one journals in the discipline of plant pathology were selected by the
four authors as being the primary choice for most plant pathologists when
...
[Issue #3](https://github.com/adamhsparks/Reproducible-Research-in-Plant-Pathology/issues/3),
...
easily. Once we're done filling in our evaluations, we will import the data back
to R. Jenny Bryan has created a handy package, _googlesheets_ that we use here.
_Note that this must be run interactively for `gs_new()` to work. Knitting this
.Rmd file will not generate any gsheets, it must be done in an interactive R
session._
...
The Journal of Plant Pathology does not supply valid bib entries via the DOI.
...
cleaned <- unlist(lapply(bib_clean, clean_articles))
...
"Phytopathologia Mediterranea" also provides some malformed bib entries.

```{r second_cleaning, warning=FALSE, message=FALSE, results="hide", eval=FALSE}
# locations of Journal of "Phytopathologia Mediterranea" entries in bib object
clean2 <- grep("journal=\\{Phytopathologia Mediterranea\\}", bib)
bib_clean2 <- bib[clean2]
clean_articles2 <- function(x) {
    gsub(" @article., title=", "PM", y)
}

# apply function and clean the bib entries
cleaned <- unlist(lapply(bib_clean2, clean_articles2))
  
```

Next export the file to disk as .bib files to be cleaned using
[Bibdesk](bibdesk.sourceforge.net). This is only necessary for the "cleaned"
entries. This set of data have special characters that are not handled by R
properly, resulting in an improperly formatted bib file when importing using
`bib2df()`. All that is neeeded is to open the resulting file and resave using
Bibdesk.

```{r export_import_bib, warning=FALSE, message=FALSE, results="hide", eval=FALSE}
# write file to a temp directory and read back as a tibble
cleaned <- unlist(cleaned)

# remove any previous files from running this
unlink("~/tmp/cleaned_article_notes.bib")

# write to disk and reimport
lapply(cleaned, write, "~/tmp/cleaned_article_notes.bib", append = TRUE)
```

*Important* Open resulting , "~/tmp/cleaned_article_notes.bib" in Bibdesk and
resave. Bibdesk will fix inconsistencies in these 11 entries allowing R to
...
The Google Sheets can be found here https://docs.google.com/spreadsheets/d/1pwUnvJgcnvwg-b8vLEbrdAbIpS6Rdbd0lIiZJi2-GfI/edit?usp=sharing.
