...
```{r}
to_record <- c("doi", "Article", "Authors", "Vol", "Iss", 
"pp", "IF", "Page_charges", "Country", "Open_or_Restricted", 
"Reproducibility_instructions", "Iss_per_Year", "Article_class", 
"Supl_mats", "Comp_methods_availability", "Software_availability", 
"Software_citation", "Analysis_automation", "Data_availability", 
"Data_annotation", "Data_tidiness", "Reproducibility_score")
journals[to_record] <- ""
```

Since we don't want to overwrite our edits each time this Rmd is run, we will save this as "article_data_template.csv" and the one we edit will be called just "article_data.csv" and will live in the "data" folder.

```{r}
write.csv(journals, file = "article_data_template.csv", quote = FALSE)
```
...
bib_path <- "../inst/examined_articles.bib"
writeChar(paste0(bib, collapse = "\n\n"), bib_path)
```


```{r}
parsed_bib <- read.bib(bib_path)
bib_data <- data.frame(
  authors = vapply(parsed_bib$author, FUN.VALUE = character(1), paste0, collapse = ", "), 
  title = unlist(parsed_bib$title), 
  volume = unlist(parsed_bib$volume)
)
```
