---
title: "Assigning articles to authors"
...
We handpicked a list of 21 journals that we felt represented plant pathology research. In this step, we will create a tibble in R of these journals, assigning them a number so that we can randomise them.
...
## Join articles with assignees

The articles generated previously were searched for and DOIs were saved in a data file, `assigned_article_notes.rda`, with notes related to the articles and selections. Using the DOIs in this file, we can retrieve the BibTex citations for those articles that provide a DOI.

```{r join_files, warning=FALSE}
notes <- data(Reproducibility.in.Plant.Pathology::assigned_article_notes)

bib <- RefManageR::GetBibEntryWithDOI(notes$doi)

# Join articles and references for saving to CSV for reference
article_assignments <- dplyr::left_join(notes, as.data.frame(bib), by = "doi")

# clean up an extra year column (that reports the first online, not actual print publish year)
article_assignments <- article_assignments[, -9]
...
```{r save data}
# Save assigned_articles to data object
devtools::use_data(article_assignments)

# Save the bibiblography item to generate .bib file from later
devtools::use_data(bib)
```
