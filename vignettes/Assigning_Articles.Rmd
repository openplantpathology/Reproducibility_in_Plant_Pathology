...
bib <- unlist(bib)
lapply(bib, write, paste0(tempdir(), "/article_notes.bib"), append = TRUE)
parsed_bib <- bib2df(paste0(tempdir(), "/article_notes.bib"))
names(parsed_bib) <- tolower(names(parsed_bib))

# join notes with bibliographic data
parsed_bib <- left_join(parsed_bib, notes)
parsed_bib <- arrange(parsed_bib, assignee)

# the author and editor columns are actually a list of items, this causes issues
# writing the parsed_bib object out to a file. We can drop it for now since it's
# not necessary for the evaluation of the articles themselves.

parsed_bib <- parsed_bib[, -c(9, 12)]
