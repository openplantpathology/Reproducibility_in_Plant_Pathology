...
Jenny Bryan has created a handy package, _googlesheets_ that we use here.
_Note that this must be run interactively for `gs_new()` to work.
Knitting this .Rmd file will not generate any gsheets, it must be done in an interactive R session._
...
This step is completed in gsheets, Adam looked up articles and added notes and DOIs to a gsheet, "article_notes" which will be imported to R in the next step.
...
Next export the items as files to disk as .bib files to be cleaned using JabRef. This will automatically assign unique cite keys and fix some issues with characters in the bib file.
...
The last few steps are completed outside of R in Google Docs, manually adding the missing information to the spreadsheet evaluations of their respective articles.

The Google Sheets can be found here: https://docs.google.com/spreadsheets/d/19gXobV4oPZeWZiQJAPNIrmqpfGQtpapXWcSxaXRw1-M/edit#gid=1699540381.
