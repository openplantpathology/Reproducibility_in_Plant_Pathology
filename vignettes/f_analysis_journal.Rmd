...
rrpp <-
  within(rrpp,
         abbreviation <-
           relevel(abbreviation, ref = "Mol Plant Pathol"))
...
Test the effect that the journal may have on the availability of software used.
Here the assignee is not used as a random effect because A. Sparks evaluated all scores and adjusted them based on the recorded software used, so in essence a single evaluator assigned this score.

```{r openness}
rrpp <- import_notes()
rrpp <-
  within(rrpp,
         journal <- relevel(abbreviation,
                            ref = "Mol Plant Pathol"))
...
plot(equivalence_test(m_f3), range = c(-0.1, 0.1))
```
